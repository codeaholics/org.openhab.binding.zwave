 ...
 * Byte 3 : Message Class (see MessageClass)
...
public class ByteMessage {
    ...
    private ByteMessageType messageType;
    private int messageClassKey;
    private ByteMessagePriority priority;
    private MessageClass expectedReply;
    ...
    /**
     * Constructor. Creates a new instance of the ByteMessage class.
     */
    public ByteMessage() {
    ...
    /**
     * Constructor. Creates a new instance of the ByteMessage class using the
     ...
     */
    public ByteMessage(MessageClass messageClass, ByteMessageType messageType, MessageClass expectedReply,
            ByteMessagePriority priority) {
     ...
     * @param priority      the message priority
     */
    public ByteMessage(int nodeId, MessageClass messageClass, ByteMessageType messageType, MessageClass expectedReply,
            ByteMessagePriority priority) {
    ...
    /**
     * Constructor. Creates a new instance of the ByteMessage class from a
     * specified buffer.
     *
     * @param buffer the buffer to create the ByteMessage from.
     */
     ...
     * @param buffer the buffer to create the ByteMessage from.
     */
    public ByteMessage(int nodeId, byte[] buffer) {
        logger.trace("NODE {}: Creating new ByteMessage from buffer = {}", nodeId, ByteMessage.bb2hex(buffer));
        ...
        this.priority = ByteMessagePriority.High;
        this.messageType = buffer[2] == 0x00 ? ByteMessageType.Request : ByteMessageType.Response;
        this.messageClassKey = buffer[3] & 0xFF;
        this.messagePayload = ArrayUtils.subarray(buffer, 4, messageLength + 1);
        this.messageNode = nodeId;
        logger.trace("NODE {}: Message payload = {}", getMessageNode(), ByteMessage.bb2hex(messagePayload));
    }
    ...
    /**
     * Returns a string representation of this ByteMessage object.
    ...
    public String toString() {
        return String.format(
                ...
                new Object[] { MessageClass.getMessageClass(messageClassKey), messageClassKey, messageType,
                        messageType.ordinal(), priority, messageNode, getCallbackId(),
                        ByteMessage.bb2hex(this.getMessagePayload()) });
    };

    /**
     * Gets the ByteMessage as a byte array.
     *
     * @return the message
     */
    public byte[] getMessageBuffer() {
        ...
        final MessageClass messageClass = MessageClass.getMessageClass(messageClassKey);
        int messageLength = messagePayload.length
                + (messageClass == MessageClass.SendData && this.messageType == ByteMessageType.Request ? 5 : 3); // calculate
                                                                                                                  // and
                                                                                                                  // set
                                                                                                                  // length
        ...
        if (messageClass == MessageClass.SendData && this.messageType == ByteMessageType.Request) {
        ...
        logger.debug("Assembled message buffer = " + ByteMessage.bb2hex(result));
        return result;
    }
     ...
     */
    public ByteMessageType getMessageType() {
        return messageType;
    }
    ...
    public MessageClass getMessageClass() {
        return MessageClass.getMessageClass(messageClassKey);
    }
     ...
     */
    public int getMessagePayloadByte(int index) throws ZWaveByteMessageException {
        if (index >= messagePayload.length) {
            throw new ZWaveByteMessageException(
     ...
     */
    public MessageClass getExpectedReply() {
     ...
     * @return the priority
     */
    public ByteMessagePriority getPriority() {
        return priority;
    }
     ...
     */
    public void setPriority(ByteMessagePriority p) {
        priority = p;
    }
    ...
    /**
     * Byte message type enumeration. Indicates whether the message is a request or a response.
     *
     */
    ...
    /**
     * Byte message priority enumeration. Indicates the message priority.
     ...
     *
     */
    public enum ByteMessagePriority {
        RealTime,
        Immediate,
        High,
        Set,
    ...
    /**
     * Message class enumeration. Enumerates the different messages that can be exchanged with the controller.
     *
     */
    public enum MessageClass {
         ...
         */
        private static Map<Integer, MessageClass> codeToMessageClassMapping;

        private int key;
        private String label;

        private MessageClass(int key, String label) {
            this.key = key;
            this.label = label;
        }

        private static void initMapping() {
            codeToMessageClassMapping = new HashMap<Integer, MessageClass>();
            for (MessageClass s : values()) {
     ...
     * sequence number.
     */
    public static class MessageComparator implements Comparator<ByteMessage> {

        private final ZWaveController controller;

        /**
         * Constructor. Creates a new instance of the MessageComparator class.
         *
         * @param controller the {@link ZWaveController to use}
         */
        public MessageComparator(ZWaveController controller) {
         ...
         */
        @Override
        public int compare(ByteMessage arg0, ByteMessage arg1) {
            ...
            if ((arg0.getMessageClass() == MessageClass.RequestNodeInfo
                    || arg0.getMessageClass() == MessageClass.SendData)) {
            ...
            if ((arg1.getMessageClass() == MessageClass.RequestNodeInfo
                    || arg1.getMessageClass() == MessageClass.SendData)) {
