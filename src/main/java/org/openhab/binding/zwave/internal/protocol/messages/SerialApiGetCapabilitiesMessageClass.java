...
public class SerialApiGetCapabilitiesMessageClass extends ZWaveCommandProcessor {
    ...
    private Set<ByteMessage.MessageClass> apiCapabilities = new HashSet<>();

    public ByteMessage doRequest() {
        return new ByteMessage(MessageClass.SerialApiGetCapabilities, ByteMessageType.Request,
                MessageClass.SerialApiGetCapabilities, ByteMessagePriority.High);
    }
        ...
        for (int by = 8; by < incomingMessage.getMessagePayload().length; by++) {
            for (int bi = 0; bi < 8; bi++) {
                if ((incomingMessage.getMessagePayloadByte(by) & (0x01 << bi)) != 0) {
                    ByteMessage.MessageClass msgClass = ByteMessage.MessageClass
    ...
    public Set<MessageClass> getApiCapabilities() {
